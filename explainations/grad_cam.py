...
def show_cam_on_image(img, mask, alpha):
    """
    alpha used for decrease the strong of cam
    """
...
def show_grad_cam(image_path):
    ...
    grad_cam = GradCam(model=model.backbone, feature_module=model.backbone.features,
                    target_layer_names=['denseblock4'], use_cuda=False)
    ...
    cam = show_cam_on_image(img, grayscale_cam,alpha=0.1)
